  ...
  dashboardBody(
    tabItems(
      ...
      tabItem(tabName = "Geomline",
              ...
              box(radioButtons(inputId = "geschlecht_slider2", label = "Geschlecht", choices = c("Alle","m", "w", "NA")), height=220),
              box(plotOutput("geomline_1", height = 400, width = 400)),
              box(selectInput(inputId = "jahr_slider",label="Jahr",choices = c("Alle","2021","2020","2019","2018","2017","2016","2015","2014","2013","2012","2011")),
              plotOutput("geomline_2", height = 400, width = 400))
      )
    )
  )
)
server <- function (input, output) {
  ...
  output$geomline_1 <- renderPlot({
    ...
    BewerbungenGeschlechtJahr33 <- filter(BewerbungenGeschlechtJahr3, geschlecht==input$geschlecht_slider)
    #Geom_line
